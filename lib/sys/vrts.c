...
{
  if(vrts_state.count >= VRTS_THREAD_LIMIT - 1) return false;
  VRTS_Task_t *thread = &vrts_state.threads[vrts_state.count];
...
void VRTS_Init(void)
{
  ...
  vrts_now_thread = &vrts_state.threads[vrts_state.i];
...
void let(void)
{
  ...
  vrts_state.i++;
  if(vrts_state.i >= vrts_state.count)
    vrts_state.i = 0;
  vrts_next_thread = &vrts_state.threads[vrts_state.i];
